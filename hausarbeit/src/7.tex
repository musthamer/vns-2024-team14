...
Während unserer Tests haben wir die Befehle docker stats und docker ps verwendet, um die Container besser zu überwachen. Mit docker stats konnten wir in Echtzeit sehen, wie viel CPU und Arbeitsspeicher die einzelnen Container nutzen, was uns geholfen hat, Probleme wie Überlastungen schnell zu erkennen. Der Befehl docker ps zeigte uns zudem eine Übersicht über alle laufenden Container und deren Status, was besonders nützlich war, um den Überblick zu behalten und Fehler im System schneller zu finden.
...
\subsection{Optimierung der Webanwendung und Infrastruktur}
Während der Tests und Nutzung unserer Webanwendung haben wir festgestellt, dass die CPU-Auslastung stark anstieg und alle verfügbaren Ressourcen vollständig beansprucht wurden. Dies führte dazu, dass die Leistung unserer Anwendung erheblich beeinträchtigt wurde und Verzögerungen entstanden. Um diese Problematik zu lösen, haben wir gezielte Maßnahmen zur Optimierung der Systemleistung und zur Reduzierung der CPU-Last ergriffen.
\subsubsection{CPU-Begrenzung für Docker-Container}
Um die Überlastung der CPU zu vermeiden und die Systemstabilität zu gewährleisten, haben wir die CPU-Ressourcen für die Docker-Container begrenzt. Dies stellte sicher, dass kein einzelner Container zu viele Ressourcen beansprucht und andere Container oder Prozesse blockiert werden. Durch diese Maßnahme konnten wir die CPU-Last kontrollieren und eine gleichmäßige Verteilung der Rechenkapazität gewährleisten.
\subsubsection{Einsatz von Redis als Cache}
Ein weiterer Schritt zur Optimierung unserer Webanwendung war die Implementierung von Redis als Cache-System. Da wir festgestellt haben, dass die häufigen Anfragen an die MariaDB-Datenbank die Systemleistung erheblich beeinträchtigen, haben wir einen Zwischenspeicher (Cache) eingeführt. Redis speichert dabei häufig benötigte Daten temporär im Arbeitsspeicher, wodurch die Abfragen an die Datenbank deutlich reduziert werden. Dies führte zu einer spürbaren Verbesserung der Antwortzeiten und einer geringeren Belastung der MariaDB-Datenbank.
\subsubsection{Load Balancing mit Round Robin}
Um die Lastverteilung der eingehenden Anfragen weiter zu optimieren, haben wir ein Load Balancing-System mit der Round-Robin-Methode implementiert. Diese Methode sorgt dafür, dass eingehende Anfragen gleichmäßig auf mehrere Webserver (Apache-Container) verteilt werden. Dies verhindert, dass einzelne Container überlastet werden und erhöht die Verfügbarkeit und Stabilität der Anwendung.
\subsubsection{Skalierung der Apache-Container}
Zusätzlich zum Load Balancing haben wir unsere Infrastruktur erweitert und insgesamt drei Apache-Container erstellt. Durch diese Maßnahme wird die Last nicht nur auf einen, sondern auf drei unabhängige Container verteilt. Dies sorgt für eine gleichmäßigere Verteilung der Anfragen und reduziert die Wahrscheinlichkeit von Engpässen und Verzögerungen.
\subsubsection{Ergebnis der Optimierung}
Mit diesen Maßnahmen der CPU-Begrenzung, dem Einsatz von Redis zur Caching-Optimierung, dem Load Balancing mit Round Robin und der Skalierung auf drei Apache-Container konnten wir die Performance unserer Webanwendung erheblich verbessern. Die Anwendung reagiert nun schneller, bleibt stabiler und ist besser gegen Lastspitzen gewappnet. Diese Kombination aus Maßnahmen hat unsere Infrastruktur robuster und skalierbarer gemacht und ermöglicht eine effizientere Nutzung der Anwendung.

\subsubsection{Automatische Skalierungs der Apache-Container}

Eine mögliche Optimierung des Systems wäre ein automatisches System zu erstellen, welches neue Apache-Server hinzufügt, wenn die CPU-Auslastung zu hoch wird. Dieses System würde ständig die CPU-Last der Apache-Server mit einem Watcher-Skript überwachen. Sobald einer der Server eine CPU-Auslastung von bspw. 80\% erreicht, würde das System automatisch einen neuen Apache-Container starten. Dieser neue Server würde dann direkt in HAProxy eingebunden werden, damit der Datenverkehr gleichmäßig auf alle Apache-Server verteilt wird. Dadurch sinkt die Auslastung jedes einzelnen Apache-Servers, und das System bleibt weiterhin stabil. So könnte das System auch bei vielen Nutzern flüssig arbeiten und Wartezeiten drastisch verkürzen. Mit dieser Lösung würde sich das System automatisch an die aktuelle Belastung anpassen und für eine bessere Leistung sorgen.
