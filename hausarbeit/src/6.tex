...
Ein Beispiel für diese Caching-Logik zeigt sich in der folgenden Implementierung: 
Zunächst wird ein Schlüssel (\texttt{REDIS\_KEY}) definiert, der die gecachten ToDo-Daten repräsentiert. 
Mit dem Befehl 
...
wird versucht, die Daten direkt aus dem Cache abzurufen. Falls dieser leer ist, erfolgt der Zugriff auf die MariaDB-Datenbank mittels

\begin{verbatim}
mariadb --defaults-file=my.cnf
\end{verbatim}

um die aktuellen Daten zu laden. Die abgerufenen Daten werden dann mit 

\begin{verbatim}
redis-cli setex "$REDIS_KEY" 300 "Daten"
\end{verbatim}

für 5 Minuten (300 Sekunden) im Redis-Cache gespeichert. Dies gewährleistet, dass nachfolgende Anfragen innerhalb dieser Zeit direkt aus dem Cache bedient werden und die Datenbank entlastet wird.
...
\begin{code}{Session-ID extrahieren}{session}
        \begin{minted}{latex}
