...
\begin{code}{Apache start.sh}{apache start}
        \begin{minted}{latex}
#!/bin/bash
...
\begin{code}{Apache Dockerfile}{apache docker}
        \begin{minted}{latex}
FROM ubuntu
...
\begin{code}{Apache myinit.sh}{apache Initialisierung}
        \begin{minted}{latex}
#!/bin/bash
log=/docker.log
echo starte >> $log
...
\begin{code}{HAProxy Dockerfile}{haproxy dockerfile}
        \begin{minted}{latex}
FROM ubuntu
RUN apt-get -y update && apt-get -y upgrade
...
\begin{code}{HAProxy myinit.sh}{haproxy Initialisierung}
        \begin{minted}{latex}
...
\begin{code}{HAProxy Frontend}{haproxy.cfg}
        \begin{minted}{latex}

frontend http-in
  bind :80
  acl url_direct path_beg /direct
...
\begin{code}{HAProxy Backend}{haproxy.cfg}
        \begin{minted}{latex}
...
\begin{code}{Erstellen der Datenbank}{todo\_app}
        \begin{minted}{latex}
...
GRANT ALL PRIVILEGES ON todoa_pp.* TO 'dbuser'@'%';
FLUSH PRIVILEGES;
\end{minted}
\end{code}
...
\begin{code}{Erstellen der Tabelle todos}{todos}
        \begin{minted}{latex}
...
\begin{code}{Erstellen der Tabelle users}{users}
        \begin{minted}{latex}
CREATE TABLE users (
...
\begin{code}{Festlegen der Login-Daten}{login}
        \begin{minted}{latex}
...
\subsection{Redis}
Die Redis-Integration in der ToDo-Listen-Anwendung wird zur Verwaltung von Nutzersitzungen eingesetzt und als Cache implementiert. Damit ist Redis eine Wichtige Komponente in unserem Netzwerk, da sie gleich mehrere fundamentale Aufgaben übernimmt, die unsere Anwendung insgesamt effizienter arbeiten lässt und stabilisiert.

\subsubsection{Redis als Cache}
In der Webanwendung wurde Redis als Cache implementiert, um die Performance zu optimieren und die Zugriffszeiten auf die Daten aus der MariaDB-Datenbank zu verkürzen. Dabei verfolgt die Implementierung eine effektive Strategie zur Zwischenspeicherung von häufig abgerufenen Daten. Wenn eine Anfrage gestellt wird, erfolgt zunächst eine Überprüfung, ob die benötigten Daten bereits im Redis-Cache vorhanden sind. Ist dies der Fall, werden die Daten direkt aus dem Cache bezogen, wodurch der Zugriff sehr schnell erfolgt. Falls die Daten im Cache nicht vorhanden sind, erfolgt ein regulärer Zugriff auf die MariaDB-Datenbank, um die angeforderten Informationen abzurufen. Die erhaltenen Daten werden anschließend mit einer definierten Gültigkeitsdauer im Redis-Cache gespeichert, sodass nachfolgende Anfragen schneller beantwortet werden können.

\begin{code}{Cache-Code}{Redis-Cache}
        \begin{minted}{latex}
...
Ein Beispiel für diese Caching-Logik zeigt sich in der folgenden Implementierung: Zunächst wird ein Schlüssel (REDIS\_KEY) definiert, der die gecachten ToDo-Daten repräsentiert. Mit dem Befehl redis-cli get "$REDIS\_KEY" wird versucht, die Daten direkt aus dem Cache abzurufen. Falls dieser leer ist, erfolgt der Zugriff auf die MariaDB-Datenbank mittels mariadb --defaults-file=my.cnf, um die aktuellen Daten zu laden. Die abgerufenen Daten werden dann mit redis-cli setex für 5 Minuten (300 Sekunden) im Redis-Cache gespeichert. Dies gewährleistet, dass nachfolgende Anfragen innerhalb dieser Zeit direkt aus dem Cache bedient werden und die Datenbank entlastet wird.

Der Einsatz von Redis als Cache bietet mehrere Vorteile. Zum einen reduziert diese Methode die Last auf der MariaDB-Datenbank, da wiederholte Abfragen vermieden werden. Zum anderen verbessert sich die Gesamtleistung der Webanwendung durch schnellere Antwortzeiten erheblich. Dies ist insbesondere bei stark frequentierten Systemen von Bedeutung, da Redis durch seine In-Memory-Struktur extrem kurze Zugriffszeiten ermöglicht. Durch diese Implementierung wird eine effiziente und skalierbare Architektur geschaffen, die auch bei hohen Anfragenlasten stabile und schnelle Antwortzeiten gewährleistet.

\subsubsection{Redis als Sessionverwaltung}
Anstatt Sessions in MariaDB zu speichern, wurde Redis als schnelle und effiziente In-Memory-Datenbank verwendet, um Login-Sitzungen zu verwalten. Dies verbessert die Performance und Skalierbarkeit der Anwendung erheblich, da Redis Zugriffe wesentlich schneller verarbeitet als jedes Mal die Datensätze aus der Datenbank abzurufen um zu überprüfen ob die aktuelle Sitzung noch läuft. Beim Login eines Nutzers wird zunächst überprüft, ob die eingegebenen Zugangsdaten in der MariaDB-Datenbank vorhanden sind. Falls der Benutzername und das Passwort korrekt sind, wird eine eindeutige Session-ID generiert, die anschließend mit einer definierten Ablaufzeit in Redis gespeichert wird. Dabei wird eine eindeutige Session-ID generiert und in Redis gespeichert. Der folgende Befehl zeigt, wie die Session mit einer TTL (Time-To-Live) von 3600 Sekunden gespeichert wird:

\begin{code}{Session erstellen}{uuidgen}
        \begin{minted}{latex}
SESSION_ID=(uuidgen)redisresult=(uuidgen)redisresult=(redis-cli -h "REDISHOST"−p"REDISHOST"−p"REDIS_PORT" -a "REDISPASSWORD" SETEX"session:REDISPASSWORD" SETEX"session:SESSION_ID" 3600 "$USERNAME")
  \end{minted}
\end{code}
...
\begin{code}{Erfolgreiche Anmeldung}{cookie}
        \begin{minted}{latex}
...
\begin{code}{Login-Fehler}{Login}
        \begin{minted}{latex}
echo "<html>
<head><title>Login Failed</title></head> <body> <h1>Login fehlgeschlagen</h1> <a href='/index.html'>Erneut versuchen</a> </body> </html>"
  \end{minted}
\end{code}
...
\begin{code}{Cookie-Extraktion}{extraktion}
        \begin{minted}{latex}
session_id=""
if [ -n "HTTP_COOKIE"]; then
          sessionid=$(echo "$HTTP_COOKIE" | sed -n 
          's/.*session_id=\([^;]*\).*/\1/p')
fi
  \end{minted}
\end{code}
...
\begin{code}{Abgelaufene Sitzung}{Sitzung}
        \begin{minted}{latex}
...
\begin{code}{Session-Prüfung}{prüfen}
        \begin{minted}{latex}
user=(redis−cli−h"(redis−cli−h"REDIS_HOST" -p "REDISPORT"−a"REDISPORT"−a"REDIS_PASSWORD" GET "session:$session_id")
  \end{minted}
\end{code}
...
\begin{code}{session-ID extraktion}{session}
        \begin{minted}{latex}
...
\begin{code}{Löschen der ID}{delete}
        \begin{minted}{latex}
if [ -n "SESSIONID"];thenredis−cli−h"SESSIONID"];thenredis−cli−h"REDIS_HOST" -p "REDISPORT"−a"REDISPORT"−a"REDIS_PASSWORD" DEL "session:$SESSION_ID" >/dev/null
fi
  \end{minted}
\end{code}
...
\begin{code}{Session-ID wird auf null gesetzt}{null}
        \begin{minted}{latex}
echo "Content-type: text/html"
...
\begin{code}{Weiterleitung zum Login}{Weiterleiten}
        \begin{minted}{latex}
echo "Status: 303 See Other"
echo "Location: /index.html"
echo ""
