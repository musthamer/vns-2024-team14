...
if [ -n "$SESSION_ID" ]; then
  ESCAPED_SESSION=$(mariadb --defaults-file=my.cnf -sN -e "SELECT QUOTE('$SESSION_ID')")

  SESSION_VALID=$(mariadb --defaults-file=my.cnf -sN -e "
    SELECT name
    FROM sessions
...
<body>
  <div class='container'>
    <div style='float:right;'>
      Angemeldet als: $SESSION_VALID
      <form action='logout.sh' method='post'>
